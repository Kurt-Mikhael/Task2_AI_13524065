...
Pada pergantian jadwal perkuliahan, sejumlah kelompok mahasiswa harus berpindah tempat secara bersamaan. Sebagai contoh, beberapa kelompok harus bergerak pada waktu yang sama.
...
Sebagian besar tugas Local Search (N-Queens, Traveling Salesman Problem, penjadwalan mata kuliah, knapsack, pewarnaan graf, dan sejenisnya) menilai sebuah state berdasarkan susunan atau kombinasinya saja. Pada \textbf{CampusFlow}, sebuah state tidak hanya dinilai berdasarkan susunannya, tetapi harus dijalankan melalui \emph{simulasi waktu}. Nilai objective function dihasilkan oleh perilaku dinamis selama simulasi. Hal yang diamati meliputi jumlah mahasiswa pada satu koridor di satu menit tertentu, pertemuan dua kelompok dari arah berlawanan, dan ketepatan waktu kedatangan.
...
Sebagian constraints bersifat wajib (item 1--7). State yang melanggarnya dinyatakan tidak valid dan tidak pernah diproses lebih lanjut. Sebagian lainnya bersifat prioritas (item 8--13). Constraint tersebut boleh dilanggar, tetapi setiap pelanggaran menambah nilai cost.
...
Initial state dibuat secara acak. Untuk setiap kelompok, program melakukan langkah berikut.
...
Pembangkitan initial state ditangani oleh fungsi \texttt{generate\_initial\_state()}. State random pertama biasanya belum efisien karena beberapa kelompok dapat menumpuk pada jalur yang sama. Contoh initial state dari POC menggunakan seed tetap 42 agar reprodusibel.
...
Untuk setiap ruas dan setiap waktu, program menghitung okupansi dan arah perjalanan.

\[
OverCapacity(e, t) = \max(0,\ Occupancy(e, t) - Capacity(e))
\]

Total penalti kapasitas dihitung sebagai berikut.

\[
C_{\mathrm{capacity}} = \sum_t \sum_e OverCapacity(e, t)^2
\]

Nilai dikuadratkan agar kelebihan kapasitas yang besar mendapat hukuman lebih berat. Sebagai contoh, koridor berkapasitas 40 yang diisi 55 mahasiswa menghasilkan penalti $(55-40)^2 = 225$.
...
Tahapan Basic pada setiap pencarian adalah sebagai berikut.
...
dengan $\Delta E$ adalah peningkatan cost dan $T$ adalah temperature. Tahapannya sebagai berikut.
...
Population berisi sejumlah state random dengan ukuran 40 pada POC. Karena persoalan menggunakan cost minimization, fitness dihitung sebagai berikut.
...
Metode yang digunakan adalah \textbf{Tournament Selection} dengan ukuran turnamen 3. Chromosome dengan fitness tertinggi dipilih dari beberapa kandidat acak. Tournament Selection dipilih karena lebih sederhana untuk proof of concept.
...
Mutation berupa salah satu neighbor move. Operasinya dapat mengganti jalur satu kelompok, menambah atau mengurangi delay, atau menukar delay dua kelompok. POC menggunakan probabilitas mutasi sebesar $0{,}4$.

% ===================== SECTION 10 =====================
\section{Daftar Fungsi dan Command Utama Python}

Implementasi dibagi menjadi beberapa modul pada folder \texttt{src/local\_search/} agar mudah dikembangkan dan diuji. Struktur modul dijelaskan pada tabel berikut.
...
\hline
\texttt{src/local\_search/main.py} & Titik masuk yang menjalankan seluruh algoritma. \\
\hline
\end{tabularx}

Fungsi-fungsi berikut diimplementasikan pada modul \texttt{src/local\_search/} dan dijalankan dari direktori repositori.
...
\begin{itemize}[leftmargin=2em,itemsep=0.1em]
  ...
  \item \texttt{src/local\_search/main.py} dan modul pendukung \texttt{src/local\_search/} sebagai proof of concept Python.
...
CampusFlow merupakan persoalan local search yang lengkap. Rancangan ini memiliki complete-state formulation yang jelas, neighbor function berbasis empat move sederhana, objective function multikriteria enam komponen, serta peran yang relevan untuk empat varian Hill-Climbing, Simulated Annealing, dan Genetic Algorithm. Proof of concept membuktikan algoritma berhasil menurunkan cost dari 531{,}38 menjadi 8{,}00 (perbaikan 98{,}49\%) pada konfigurasi uji. Rancangan ini dapat dikembangkan lebih lanjut menjadi simulasi keamanan, evakuasi, atau manajemen fasilitas kampus tanpa mengubah kontrak fungsi utama.
...
Kode sumber lengkap POC dibagi menjadi tujuh modul pada folder \texttt{src/local\_search/}. Untuk menjalankan seluruh simulasi, gunakan \texttt{python src/local\_search/main.py}. Berikut isi setiap modul.
